    ...
    <h2 class="mb-4 text-xl font-bold text-black-900 dark:text-white">Ubah</h2>
      <form action="proses-ubah.php" method="POST">
          <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
              <div class="w-full">
                  <label for="namalengkap" class="block mb-2 text-sm font-medium text-black-900 dark:text-white">NamaPenerbit</label>
                  <input type="text" name="NamaPenerbit" value="<?= $Penerbit->NamaPenerbit?>" id="NamaPenerbit" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Masukkan NamaPenerbit Lengkap" required="">
              </div>
              <div class="sm:col-span-2">
                  <label for="alamat" class="block mb-2 text-sm font-medium text-black-900 dark:text-white">Alamat</label>
                  <textarea name="alamat" id="alamat" rows="8" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Enter Your Address"> <?= $Penerbit->Alamat?></textarea>
              </div>
              <div class="sm:col-span-2">
                  <label for="notelp" class="block mb-2 text-sm font-medium text-black-900 dark:text-white">Nomor Telepon</label>
                  <input type="text" name="notelp" value="<?= $Penerbit->NoTelp?>" id="notelp" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Masukkan Nomor Telepon" required="">
              </div>
              <div>
                  <input type="hidden" name="id" id="id" value="<?= $Penerbit->ID ?>" >
                </div>
              <div class="sm:col-span-2">
